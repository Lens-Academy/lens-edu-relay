...
Two AIs are trained to find their way around cities. One ends up memorizing every route it has ever walked. The other builds a fresh map of each new city and follows a course through it. Drop them both in a city neither has seen before. By the chapter's account, which one is starting to "want" to reach the destination — and what exactly are you pointing at when you say so?
...
The question they were asked is a deliberate wedge — it is not the test question. It hands the student two similarly trained AIs that differ in one respect (memorizing routes vs. building and using a map), and asks them to apply the chapter's account of where wanting comes from rather than recite it. Use it to draw out separable prediction and steering skills, and especially the point that the proto-want lives in using the map to steer, not merely in having one.
