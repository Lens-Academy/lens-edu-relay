...
A friend says: "An AI only 'wants' something if its programmers wrote that goal into it. Nobody coded a goal into a system that just learned from examples — so a trained AI doesn't really want anything, it's only matching patterns." Using Chapter 3, where does this reasoning break down? Be precise about what you do and don't have to claim about the AI's inner life.
...
The question they were asked is a deliberate wedge — it is not the test question. It hands the student a plausible-sounding objection (that wanting must be explicitly programmed in, and that a system which merely learned from examples is therefore "just matching patterns") and asks them to dismantle it using the chapter's account of where wanting comes from, rather than recite that account. Use it to draw out two distinct points: that want-like behavior emerges as a side effect of training for success rather than being designed in (the city-navigation mechanism, the o1 incident), and that "wanting" here names outward steering behavior — so the rebuttal does not require, and should not rest on, any claim about the AI's inner experience.
