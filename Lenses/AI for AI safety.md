---
id: ace55d5a-2265-442c-a9d3-e8b6106a9078
tldr: There may be a window where AI is smart enough to help with safety research but not yet powerful enough to be uncontrollable. This article explores the idea of using AI to strengthen our safety tools before models outpace our ability to oversee them — a race between two feedback loops.
tags:
  - lens
---
### Article: AI for AI safety
source:: [[../articles/carlsmith-ai-for-ai-safety]]

#### Text
content::
This post argues there's a temporary "AI for AI safety" sweet spot. This is a period where AI is smart enough to help with safety but not yet smart enough to be uncontrollable. The theory of change relies on differential progress (i.e. safety progress moves faster than . We must use AI to strengthen safety factors. This should happen before the models reach a level of superintelligence.


#### Article-excerpt
from:: "*(Audio version* [*here*]"
to:: "a few options in this respect, for the different security factors above."


#### Article-excerpt
from:: "2.1 A tale of two feedback loops"
to:: "restraint enough to hold off on pushing the capability frontier when necessary)."

#### Text
content::
Ask the AI Tutor any questions you may have:

#### Chat
instructions::
Help the user understand this article, or help them with other questions they have.

