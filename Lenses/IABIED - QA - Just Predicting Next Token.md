---
id: 71d3ce4e-68ce-401e-9f56-9f28fb8fe880
title: "Aren't AIs just predicting the next token?"
tags:
  - lens
  - IABIED
  - supplementary
  - work-in-progress
---

#### Text
content::
This Q&A argues that "just predicting the next token" actually requires understanding the world. If an AI accurately predicts a physicist's novel answer, it must reason about physics at that level. Furthermore, modern AIs are trained for task completion through reinforcement learning, not just prediction.

#### Article
source:: [[../articles/iabied-ch2-faq-next-token]]
to:: "simple computational foundations"

#### Text
content::
What do you think? Does this address a concern you had, or raise new questions?

#### {--{"author":"AI","timestamp":1774006476299}@@Chat: Discussion--}{++{"author":"AI","timestamp":1774006476299}@@Chat++}
instructions::
The student just read a supplementary Q&A from the book's website about whether AIs are just predicting the next token.

TLDR: Predicting tokens well requires world-understanding — accurately predicting a physicist's novel answer means reasoning at that level. Modern AIs go beyond prediction through reinforcement learning for task completion. Even pure prediction training likely induces steering behavior.

Discussion topics:
- The argument that "predicting a physicist's novel answer requires reasoning like a physicist" — do you find this convincing, or could memorization and pattern-matching explain the same outputs?
- How does the shift from prediction training to reinforcement learning change the nature of what these systems are doing?

Ask what they found surprising or new.