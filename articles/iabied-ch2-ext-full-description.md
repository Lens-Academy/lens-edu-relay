...
[¶](#ftnt72_ref) As another aside about the attention layer, Llama uses "causal masking," meaning that each token's queries can only look at the keys ++}from {--{"author":"AI","timestamp":1774265546930}@@website--}{++{"author":"AI","timestamp":1774265546930}@@*before* it. Basically, that's because each token is trying to ultimately predict what token comes next; looking ahead would be cheating!
